...
const Form = () => {
    ...
    const fetchAgeGroups = async (medicineName) => {
        try {
            const response = await fetch(`http://localhost:8080/records/age_groups?medicine=${medicineName}`);
            const data = await response.json();
            setAgeGroups(data);
        } catch (error) {
    ...
    const fetchMeasure = async (medicineName, ageGroup) => {
        try {
            const response = await fetch(`http://localhost:8080/records/measure?medicine=${medicineName}&ageGroup=${ageGroup}`);
            const data = await response.json();
    ...
    const submitData = async () => {
        try {
            const response = await fetch('http://localhost:8080/records/add_medicine', {
                method: 'POST',
                headers: {
    ...
    return (
        <div className="formdiv">
                <select className="form-select" aria-label="Default select example" onChange={handleMedicineChange} disabled={disabled}>
                    <option selected>Select Available Medicine</option>
                    {medicines.map(med => (
                        <option key={med._id} value={med.medicine_name}>{med.medicine_name}</option>
                    ))}
                    <option value="Custom">New Medicine</option>
                </select>
                <select className="form-select" aria-label="Default select example" onChange={handleAgeGroupChange} disabled={disabled}>
                ...
                {disabled && (
                    <div className='newmed'>
                        <input type="text" placeholder="Enter New Medicine" onChange={(e) => setNewMedicine(e.target.value)} />
                        <input type="text" placeholder="Enter Age Group" onChange={(e) => setNewAgeGroup(e.target.value)} />
                        <input type="text" placeholder="Enter Dosage" onChange={(e) => setNewMeasure(e.target.value)} />
                        <textarea name="" id="" cols={30} rows={5} placeholder='Enter Decsription for medicine' onChange={(e)=>(setDescription(e.target.value))}></textarea>
